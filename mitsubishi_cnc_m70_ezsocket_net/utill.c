...
{
	int64 temp = *(int64*)&i;
	big_nt_to_bytes(temp, bytes);
}

double bytes_to_double(byte* bytes)
{
	int64 temp = bytes_to_big_int(bytes);
	return *(double*)&temp;
}
...
- str       Destination text; should be 66 characters long for radix2, 24 - radix8, 22 - radix10, 18 - radix16.
...
{
	bool is_ok = true;
	short int a = 0x1122; // Hexadecimal, each value occupies 4 bits
	char b = *(char*)&a;
	if (b == 0x11)
		is_ok = false;
